...
static void draw_sprites_while1(t_game *game, t_sprite *sprite)
{
  ...
  sprite->transform_x = sprite->inv_det * ((sprite->dir_y * sprite->sprite_x) - (sprite->dir_x * sprite->sprite_y));
  sprite->transform_y = sprite->inv_det *
  (game->world->plane_y * sprite->sprite_x +
    game->world->plane_x * sprite->sprite_y);
  sprite->sprite_screen_x = (int)((game->window->width / 2) * (1 + sprite->transform_x / sprite->transform_y));
...
{
  sprite->texture_x = ((float)(x - sprite->draw_start_x) / (float)(sprite->draw_end_x - sprite->draw_start_x)) * game->world->texture_sprite->width;
  sprite->texture_y = (float)(y - sprite->draw_start_y) / (float)(sprite->draw_end_y - sprite->draw_start_y) * game->world->texture_sprite->height;
  get_pixel_color(game->world->texture_sprite, sprite->texture_x, sprite->texture_y, sprite->color);
